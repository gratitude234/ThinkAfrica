...
export default async function FollowersPage({ params }: PageProps) {
  ...
  const { data: followsRaw } = await supabase
    .from("follows")
    .select("follower_id, profiles!follows_follower_id_fkey(id, username, full_name, university, avatar_url)")
    .eq("following_id", profile.id)
    .order("created_at", { ascending: false });

  const followers = (followsRaw ?? []).map((f) => {
  ...
  return (
    <div className="max-w-2xl mx-auto">
      <div className="mb-6">
        ...
        <p className="text-gray-500 text-sm mt-1">{followers.length} follower{followers.length !== 1 ? "s" : ""}</p>
      </div>
        ...
        <div className="space-y-3">
          {followers.map((follower) => (
            ...
            >
              ...
              <div>
                <p className="font-medium text-gray-900">{follower!.full_name}</p>
