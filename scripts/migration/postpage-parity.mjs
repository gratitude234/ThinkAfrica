/**
 * Compares the post page's PostgREST reads with its PostgreSQL reads, against
 * the SAME production database.
 *
 *   node scripts/migration/postpage-parity.mjs
 *
 * This is the check that only exists during this stage of the migration, and
 * it is the strongest one available: both sides see identical rows at the same
...
if (missing.length) {
  console.error("\nCannot run the post page parity check. Missing:\n");
...
// Resolve the pooled host once here, so the test does not have to know about
// Supavisor or IPv6.
let direct;
try {
...
const result = spawnSync(
  ...
  [
    ...
    "run",
    "lib/db/postPage.parity.live.test.ts",
    "--reporter=verbose",
  ],
  {
    stdio: "inherit",
