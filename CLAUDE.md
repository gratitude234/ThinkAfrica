...
- `FEED_VIEWER_POSTGRES_FAILOVER`: Server-only emergency fallback for the signed-in feed viewer-context read. Exact value `1` enables it; unset/`0` leaves it off. Only enable after `DATABASE_URL` is verified as a production-safe target. When `READ_MIGRATED_DOMAINS` already includes `feed`, viewer context uses that direct PostgreSQL path normally and this flag is unnecessary.
